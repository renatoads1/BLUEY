...
namespace BLUEY.Models.Repositories
{
    public class AspnetUserRolesRepository : BaseRepository,IAspnetUserRolesRepository
    {
        ...
        {
            var existe = _contextM.UserRoles.Where(a=>a.UserId == userId && a.RoleId == roleId).FirstOrDefault();
            if (existe == null) {
                _contextM.userRoles.Add(new IdentityUserRole<string>() { UserId = userId, RoleId = roleId });
